...
export class MyProfile implements OnInit {
  ...
  // ── Main Data Signals ──
  readonly activeView = signal<'main' | 'personal' | 'health' | 'emergency' | 'settings'>('main');
  ...
  get diseaseStatusLabel(): (status: number) => string {
    return (status: number) => {
      switch (status) {
        case 0: return this.t().myProfile.active;
        case 1: return this.t().myProfile.controlled;
        case 2: return this.t().myProfile.resolved;
        default: return this.t().myProfile.active;
      }
    };
  }
